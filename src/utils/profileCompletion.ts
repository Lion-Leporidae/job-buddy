...
function makeChecker(target: CompletionGroup[]) {
  return (condition: boolean, sectionId: string, sectionLabel: string, field: string) => {
    if (!condition) {
      ...
      if (!g) {
        g = { sectionId, sectionLabel, fields: [] };
        target.push(g);
      }
      g.fields.push(field);
    }
...
export function calculateCompletion(profile: Partial<Profile>): CompletionResult {
  ...
  // Languages (1)
  check((profile.languages ?? []).length >= 1, 'languages', 'Languages', 'At least one language');
  ...
  optCheck(
    !!profile.personal?.veteranStatus?.trim(),
    'personal',
    'Personal Information',
    'Veteran Status',
  );
  optCheck(
    !!profile.personal?.disabilityStatus?.trim(),
    'personal',
    'Personal Information',
    'Disability Status',
  );
  ...
  if (whFilled.length > 0) {
    optCheck(
      whFilled.every((e) => !!e.location?.city?.trim() || !!e.location?.countryCode?.trim()),
      'workHistory',
      'Work History',
      'Location',
    );
    optCheck(
      whFilled.every((e) => !!e.arrangement),
      'workHistory',
      'Work History',
      'Work Arrangement',
    );
    optCheck(
      whFilled.every((e) => !!e.description?.trim()),
      'workHistory',
      'Work History',
      'Description',
    );
